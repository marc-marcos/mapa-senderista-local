...
def gui():
    ...
    btn3 = Button(window, text="Open Map", command=lambda: webbrowser.open('index.html'))
    btn3.pack()
...
def gui_modifyStatus():
    ...
    btn = Button(window, text="To-Do", command=lambda: modifyStatusAndClose(file_path, 0, window))
    btn.pack()

    btn2 = Button(window, text="Planned", command=lambda: modifyStatusAndClose(file_path, 1, window))
    btn2.pack()

    btn3 = Button(window, text="Done", command=lambda: modifyStatusAndClose(file_path, 2, window))
    btn3.pack()
