...
def regenerateRoutes():
    ...
    # If a route is in the database but not in the folder, delete it
    for row in cursor.execute('SELECT * FROM caches'):
        if row[0] not in os.listdir(dir_path):
            cursor.execute('DELETE FROM caches WHERE cache_path=?', (row[0],))
    ...
    for path in os.listdir(dir_path):
        if path.endswith('.gpx'):
            ...
            # If a route with the same path already exists, skip this one
            if cursor.execute('SELECT * FROM caches WHERE cache_path=?', (str(path),)).fetchone() is None:
                cursor.execute(
                    'INSERT INTO caches (cache_path, cache_name, status) VALUES (?, ?, ?)', (str(path), name, 0))
