...
def backup_and_restore_gui():
    ...
    btn = Button(window, text="Backup Database", command=lambda: backupDatabase(generate_path_backup()))
    btn.pack()

    btn2 = Button(window, text="Restore Database", command=lambda: restoreProcess(window))
    btn2.pack()
